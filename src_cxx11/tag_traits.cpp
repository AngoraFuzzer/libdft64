...
std::string tag_sprint(std::vector<tag_seg> const & tag) {
	std::vector<tag_seg>::const_iterator t;
	std::stringstream ss;
  ss << "{";
  for (auto p: tag) {
    ss << "(" << p.begin << ", " << p.end << "), ";
...
TagNode* tag_traits<TagNode*>::cleared_val = nullptr;
TagNode* tag_traits<TagNode*>::set_val = nullptr;
