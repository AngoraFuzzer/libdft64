...
VOID TestHandler(ADDRINT parg) {
  u64 arg = *((u64 *)parg);
  tag_t t = tagmap_getn(parg, 8);
  LOGD("[PIN][TEST] args: %lu, taint: %s\n", arg, tag_sprint(t).c_str());
}

VOID EntryPoint(VOID *v) {

  for (IMG img = APP_ImgHead(); IMG_Valid(img); img = IMG_Next(img)) {
    RTN test_rtn = RTN_FindByName(img, "__libdft_get_taint");
    if (RTN_Valid(test_rtn)) {
      RTN_Open(test_rtn);
      RTN_InsertCall(test_rtn, IPOINT_BEFORE, (AFUNPTR)TestHandler,
