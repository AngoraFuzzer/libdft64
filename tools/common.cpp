...
void OutInit () {
  char *track_file = getenv(TRACK_OUTPUT_VAR);
  if (track_file)
...
void OutWrite(void *data, int size) {
  if (!out_f) return;
  int nr = fwrite(data, size, 1, out_f);
  if (nr == 0) {
...
void sendData(void *data, int size) {
  if (sendto(socket_fd, data, size, 0, (struct sockaddr *)&socket_dst,
    ...
    //fprintf(stderr, "fail to call sendto\n");
  }
}
...
void SocketInit() {
  ...
  if (socket_fd < 0) {
    fprintf(stderr, "fail to creat socket fd\n");
    exit(1);
  }
