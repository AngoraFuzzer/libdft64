...
bool is_dup_cond(u32 cmpid, ADDRINT op1_addr, ADDRINT op2_addr) {
  ...
  u32 id = cmpid;
  std::map <u32, std::vector<ArgList> >::iterator it = __angora_unique_map.find(id);
  if (it == __angora_unique_map.end()) {
    ...
    for (it2 = it->second.begin(); it2 != it->second.end(); it2++) {
      if (it2->lb1 == lb1 && it2->lb2 == lb2 && it2->op1 == op1 && it2->op2 == op2) {
        return true;
      }
    }
    ArgList al = {op1, op2, lb1, lb2};
...
void combineTag(std::vector<tag_seg> &tag_all, ADDRINT addr, u32 size) {
  for (size_t i = 0; i < size; i++) {
    ...
    if (tag == tag_traits<tag_t>::cleared_val) continue;
#ifdef USE_TREE_TAG
...
VOID CmpHandler(u16 Cond, u32 Cid, u32 Size, u16 Type, u32 Context, ADDRINT pArg1, ADDRINT pArg2) {
  ...
  // fprintf(stderr, "[PIN][CMP] Cid: %d, Size: %d, Type: %d, Context: %d, Args : (%lld, %lld)\n",
  //         Cid, Size, Type, Context, Arg1, Arg2);

  if (ctr > MAX_COND_CTR || is_dup_cond(Cid, pArg1, pArg2)) return;
  ...
  if (tag1_size == 0 && tag2_size == 0) return;

  u64 delta = Arg1 > Arg2 ? Arg1 - Arg2 : Arg2 - Arg1;

  if (Type == COND_BOOL_TYPE) delta = 0;
...
VOID SwHandler(u32 Cid, u32 Size, u32 Context, ADDRINT pCond, u32 Num, ADDRINT pArgs) {
  u32 ctr = get_cond_ctr(Cid, Context);

  // const tag_t *tag = tagmap_getb_as_ptr(pCond);
  if (ctr > MAX_COND_CTR) return;
...
VOID SoHandler(u32 Cid, u32 Size, u16 Type, u32 Context, ADDRINT pVal) {
  ...
  if (Size == 1) stmt.extra = 0xff;
  else if (Size == 2) stmt.extra = 0xffff;
  else if (Size == 4) stmt.extra = 0xffffffff;
  else stmt.extra = 0xffffffffffffffff;
...
VOID EntryPoint(VOID *v) {
  ...
  for (IMG img = APP_ImgHead(); IMG_Valid(img); img = IMG_Next(img)) {
    ...
    if (RTN_Valid(sw_rtn)) {
      RTN_Open(sw_rtn);
      RTN_InsertCall(sw_rtn, IPOINT_BEFORE, (AFUNPTR)SwHandler,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 1,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 2,
    ...
    if (RTN_Valid(so_rtn)) {
      RTN_Open(so_rtn);
      RTN_InsertCall(so_rtn, IPOINT_BEFORE, (AFUNPTR)SoHandler,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 1,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 2,
                     IARG_FUNCARG_ENTRYPOINT_VALUE, 3,
...
/*
  ...
  void SaveOffs(const tag_t *tags, u32 size, u16 type, u32 ins_loc) { //, u32 bbid, u32 ctr
  u32 cmpid = ins_loc;
  // TODO : how to add context?
  u32 context = 0;
  // avoid duplication
  u32 ctr = get_cond_ctr(cmpid, 0);
  if (ctr > MAX_COND_CTR || is_dup_so(tags)) return;

  std::vector<tag_seg> segs;
  for (size_t i = 0; i < size; i++) {
  if (tags[i] == tag_traits<tag_t>::cleared_val) continue;
  #ifdef USE_TREE_TAG
  const std::vector<tag_seg> tag_i = TagSet::find(tags[i]);
  tag_combine_inplace(segs, tag_i);
  #else
  //tag_combine_inplace(tag_all, tag);
  #endif
  }
  ...
  OutWrite(&stmt, sizeof(stmt));
  OutWrite(&segs[0], sizeof(tag_seg) * segs.size());
  }

  VOID IndBrReg(thread_ctx_t *thread_ctx, u32 idx, u32 size, void* ins_addr) {
  //fprintf(stderr, "[PIN][INDBR] MEM\n");
  const tag_t *tag = thread_ctx->vcpu.gpr[idx];
  u32 loc = (u32)ins_addr & 0xFFFF;
  SaveOffs(tag, size, loc, COND_INDBR_TYPE);
  }

  VOID IndBrMem(ADDRINT addr, u32 size, void* ins_addr) {
  //fprintf(stderr, "[PIN][INDBR] REG\n");
  const tag_t *tag = tagmap_getb_as_ptr(addr);
  u32 loc = (u32)ins_addr & 0xFFFF;
  SaveOffs(tag, size, loc, COND_INDBR_TYPE);
  }

  VOID LeaIns(thread_ctx_t *thread_ctx, u32 idx, u32 size, void* ins_addr) {
  //fprintf(stderr, "[PIN][LEA]\n");
  const tag_t *tag = thread_ctx->vcpu.gpr[idx];
  u32 loc = (u32)ins_addr & 0xFFFF;
  SaveOffs(tag, size, loc, COND_GEP_TYPE);
  }
  ...
  if (INS_IsIndirectBranchOrCall(ins)) {
    ...
    if (INS_OperandIsReg(ins, OP_0)) {
      ...
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)IndBrReg,
      ...
      u32 arg_size = INS_OperandWidth(ins, OP_0) / 8;
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)IndBrMem,
                     IARG_MEMORYREAD_EA,
                     IARG_UINT32, arg_size,
                     IARG_INST_PTR,
                     IARG_END);
    ...
    // only can be 16/32 bit
    REG reg_dst		= INS_OperandReg(ins, OP_0);
    u32 reg_idx = 0;
    u32 arg_size = 0;
    if (REG_is_gr32(reg_dst)) {
      arg_size = 4;
      reg_idx = REG32_INDX(reg_dst);
    } else if (REG_is_gr16(reg_dst)) {
      arg_size = 2;
      reg_idx = REG16_INDX(reg_dst);
    } else {
      return;
    }
    // Should be after, so we can get the taint info
    INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)LeaIns,
                   IARG_REG_VALUE, thread_ctx_ptr,
                   IARG_UINT32, reg_idx,
                   IARG_UINT32, arg_size,
                   IARG_INST_PTR,
