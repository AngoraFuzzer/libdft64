...
void TagSet::mem_read(TagNode* const* tags, uint32_t size) {
  ...
  if (tags[0] && tags[0]->seg.begin + 1 == tags[0]->seg.end && tags[0]->prev_seg_node == NULL) {
...
void TagSet::frac_tagvec(std::vector<tag_seg> &tag_vec) {
  ...
  if (tag_vec.size() == 0) return;

  std::vector<tag_seg> tmpvec(tag_vec);
  tag_vec.clear();
  //std::cout << "vec size: " << tmpvec.size()<< "\n";
  for (std::vector<tag_seg>::iterator it = tmpvec.begin(); it != tmpvec.end(); ++it) {
...
TagNode* TagSet::combine(TagNode* node1, TagNode* node2) {
  ...
  while (!seg_st.empty()) {
    ...
    //std::cout << "seg: " << cur_seg.begin << "," << cur_seg.end << " -- " << next_seg.begin << "," << next_seg.end << "\n";
