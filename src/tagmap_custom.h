#ifndef LIBDFT_TAGMAP_CUSTOM_H
#define LIBDFT_TAGMAP_CUSTOM_H
...
#include "myarray.h"
typedef MyArray<tag_t, DIR_PAGE_SZ> tag_page_t;
typedef MyArray<tag_page_t *, DIR_TABLE_SZ> tag_table_t;
typedef MyArray<tag_table_t *, DIR_SZ> tag_dir_t;
  ...
  if (dir[virt2table(addr)]) {
    ...
    if ((*table)[virt2page(addr)]) {
      ...
      if (page != NULL) {
        ...
        // return ((tag_t*)page) + virt2offset(addr);
      }
    }
  }
  return &tag_traits<tag_t>::cleared_val;
...
inline void tag_dir_setb(tag_dir_t &dir, ADDRINT addr, tag_t const &tag) {
  ...
  if ((*table)[virt2page(addr)] == NULL) {
    ...
    // std::fill(new_page->begin(), new_page->end(),
    // tag_traits<tag_t>::cleared_val);
    new_page->fill(tag_traits<tag_t>::cleared_val);
