...
#define R64TAG(tid, RIDX)                                                      \
  {                                                                            \
    ...
    [(RIDX)][0], RTAG(tid)[(RIDX)][1], RTAG(tid)[(RIDX)][2],                   \
        ...
        RTAG(tid)[(RIDX)][6], RTAG(tid)[(RIDX)][7]                             \
  }
...
static void PIN_FAST_ANALYSIS_CALL _movsx_r2r_opqw(THREADID tid, uint32_t dst,
  ...
  tag_t src_low_tag = RTAG(tid)[src][0];
  tag_t src_high_tag = RTAG(tid)[src][1];

  /* update the destination (xfer) */
  RTAG(tid)[dst][0] = src_low_tag;
  RTAG(tid)[dst][1] = src_high_tag;
  RTAG(tid)[dst][2] = src_low_tag;
  RTAG(tid)[dst][3] = src_high_tag;
  RTAG(tid)[dst][4] = src_low_tag;
  RTAG(tid)[dst][5] = src_high_tag;
  RTAG(tid)[dst][6] = src_low_tag;
  RTAG(tid)[dst][7] = src_high_tag;
}
...
static void PIN_FAST_ANALYSIS_CALL _movzx_r2r_opqw(THREADID tid, uint32_t dst,
                                                   uint32_t src) {
  /* temporary tag value */
  tag_t src_tags[] = {RTAG(tid)[src][0], RTAG(tid)[src][1]};
...
static void PIN_FAST_ANALYSIS_CALL _movzx_m2r_opqb(THREADID tid, uint32_t dst,
  ...
  /* temporary tag value */
  tag_t src_tag = tagmap_getb(src);
  // LOG("movzx byte " + tag_sprint(src_tag) + " " + StringFromAddrint(src) + "
  // " + decstr(dst) + "\n");
  /* update the destination (xfer) */
  for (size_t i = 0; i < 8; i++)
    RTAG(tid)[dst][i] = src_tag;
...
static void PIN_FAST_ANALYSIS_CALL _cmpxchg_r2r_opq_slow(THREADID tid,
  ...
  tag_t saved_tags[] = {
      RTAG(tid)[DFT_REG_HELPER1][0], RTAG(tid)[DFT_REG_HELPER1][1],
      RTAG(tid)[DFT_REG_HELPER1][2], RTAG(tid)[DFT_REG_HELPER1][3],
      RTAG(tid)[DFT_REG_HELPER1][4], RTAG(tid)[DFT_REG_HELPER1][5],
      RTAG(tid)[DFT_REG_HELPER1][6], RTAG(tid)[DFT_REG_HELPER1][7]};
  for (size_t i = 0; i < 8; i++)
    RTAG(tid)[DFT_REG_RAX][i] = saved_tags[i];

  /* update */
  tag_t src_tags[] = {RTAG(tid)[src][0], RTAG(tid)[src][1], RTAG(tid)[src][2],
                      RTAG(tid)[src][3], RTAG(tid)[src][4], RTAG(tid)[src][5],
                      RTAG(tid)[src][6], RTAG(tid)[src][7]};
  for (size_t i = 0; i < 8; i++) {
    RTAG(tid)[dst][i] = src_tags[i];
  }
}
...
static void PIN_FAST_ANALYSIS_CALL _cmpxchg_r2r_opw_slow(THREADID tid,
  ...
  /* update */
  tag_t src_tags[] = {RTAG(tid)[src][0], RTAG(tid)[src][1]};
  RTAG(tid)[dst][0] = src_tags[0];
  RTAG(tid)[dst][1] = src_tags[1];
}
...
static ADDRINT PIN_FAST_ANALYSIS_CALL _cmpxchg_m2r_opq_fast(THREADID tid,
  ...
  tag_t save_tags[] = {RTAG(tid)[DFT_REG_RAX][0], RTAG(tid)[DFT_REG_RAX][1],
                       RTAG(tid)[DFT_REG_RAX][2], RTAG(tid)[DFT_REG_RAX][3],
                       RTAG(tid)[DFT_REG_RAX][4], RTAG(tid)[DFT_REG_RAX][5],
                       RTAG(tid)[DFT_REG_RAX][6], RTAG(tid)[DFT_REG_RAX][7]};
  for (size_t i = 0; i < 8; i++)
    RTAG(tid)[DFT_REG_HELPER1][i] = save_tags[i];
...
static void PIN_FAST_ANALYSIS_CALL _xadd_r2r_opb_ul(THREADID tid, uint32_t dst,
  ...
  RTAG(tid)[dst][1] = tag_combine(RTAG(tid)[dst][1], RTAG(tid)[src][0]);
  RTAG(tid)[src][0] = tmp_tag;
}
...
static void PIN_FAST_ANALYSIS_CALL _xadd_r2r_opb_lu(THREADID tid, uint32_t dst,
  ...
  RTAG(tid)[dst][0] = tag_combine(RTAG(tid)[dst][0], RTAG(tid)[src][1]);
  RTAG(tid)[src][1] = tmp_tag;
}
...
static void PIN_FAST_ANALYSIS_CALL _xadd_r2r_opb_u(THREADID tid, uint32_t dst,
  ...
  RTAG(tid)[dst][1] = tag_combine(RTAG(tid)[dst][1], RTAG(tid)[src][1]);
  RTAG(tid)[src][1] = tmp_tag;
}
...
static void PIN_FAST_ANALYSIS_CALL _xadd_r2r_opb_l(THREADID tid, uint32_t dst,
  ...
  RTAG(tid)[dst][0] = tag_combine(RTAG(tid)[dst][0], RTAG(tid)[src][0]);
  RTAG(tid)[src][0] = tmp_tag;
}
...
static void PIN_FAST_ANALYSIS_CALL _xadd_r2r_opw(THREADID tid, uint32_t dst,
  ...
  tag_t dst_tag[] = {RTAG(tid)[dst][0], RTAG(tid)[dst][1]};
  tag_t src_tag[] = {RTAG(tid)[src][0], RTAG(tid)[src][1]};
...
static void PIN_FAST_ANALYSIS_CALL _xadd_m2r_opb_l(THREADID tid, uint32_t src,
  ...
  RTAG(tid)[src][0] = dst_tag;
  tagmap_setb(dst, tag_combine(dst_tag, src_tag));
}

static void PIN_FAST_ANALYSIS_CALL _xadd_m2r_opw(THREADID tid, uint32_t src,
                                                 ADDRINT dst) {
  tag_t src_tag[] = R16TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL _xadd_m2r_opq(THREADID tid, uint32_t src,
                                                 ADDRINT dst) {
  ...
  RTAG(tid)[src][0] = dst_tag[0];
  RTAG(tid)[src][1] = dst_tag[1];
  RTAG(tid)[src][2] = dst_tag[2];
  RTAG(tid)[src][3] = dst_tag[3];
  RTAG(tid)[src][4] = dst_tag[4];
  RTAG(tid)[src][5] = dst_tag[5];
  RTAG(tid)[src][6] = dst_tag[6];
  RTAG(tid)[src][7] = dst_tag[7];

  tagmap_setb(dst, tag_combine(dst_tag[0], src_tag[0]));
  tagmap_setb(dst + 1, tag_combine(dst_tag[1], src_tag[1]));
  tagmap_setb(dst + 2, tag_combine(dst_tag[2], src_tag[2]));
  tagmap_setb(dst + 3, tag_combine(dst_tag[3], src_tag[3]));
  tagmap_setb(dst + 4, tag_combine(dst_tag[4], src_tag[4]));
  tagmap_setb(dst + 5, tag_combine(dst_tag[5], src_tag[5]));
  tagmap_setb(dst + 6, tag_combine(dst_tag[6], src_tag[6]));
  tagmap_setb(dst + 7, tag_combine(dst_tag[7], src_tag[7]));
}

static void PIN_FAST_ANALYSIS_CALL _xadd_m2r_opl(THREADID tid, uint32_t src,
                                                 ADDRINT dst) {
  tag_t src_tag[] = R32TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL _lea_r2r_opw(ADDRINT ins_address,
                                                THREADID tid, uint32_t dst,
...
static void PIN_FAST_ANALYSIS_CALL _lea_r2r_opq(ADDRINT ins_address,
                                                THREADID tid, uint32_t dst,
                                                uint32_t base, uint32_t index) {
...
static void PIN_FAST_ANALYSIS_CALL _lea_r2r_opl(ADDRINT ins_address,
                                                THREADID tid, uint32_t dst,
                                                uint32_t base, uint32_t index) {
...
static void PIN_FAST_ANALYSIS_CALL r2r_ternary_opb_l(THREADID tid,
                                                     uint32_t src) {
...
static void PIN_FAST_ANALYSIS_CALL r2r_ternary_opw(THREADID tid, uint32_t src) {
...
static void PIN_FAST_ANALYSIS_CALL r2r_ternary_opq(THREADID tid, uint32_t src) {
...
static void PIN_FAST_ANALYSIS_CALL r2r_ternary_opl(THREADID tid, uint32_t src) {
...
static void PIN_FAST_ANALYSIS_CALL m2r_ternary_opb(THREADID tid, ADDRINT src) {
...
static void PIN_FAST_ANALYSIS_CALL m2r_ternary_opw(THREADID tid, ADDRINT src) {
...
static void PIN_FAST_ANALYSIS_CALL m2r_ternary_opq(THREADID tid, ADDRINT src) {
...
static void PIN_FAST_ANALYSIS_CALL m2r_ternary_opl(THREADID tid, ADDRINT src) {
...
static void PIN_FAST_ANALYSIS_CALL r2m_binary_opw(THREADID tid, ADDRINT dst,
  ...
  tag_t res_tag[] = {tag_combine(dst_tag[0], src_tag[0]),
                     tag_combine(dst_tag[1], src_tag[1])};
  tagmap_setb(dst, res_tag[0]);
  tagmap_setb(dst + 1, res_tag[1]);
}
...
static void PIN_FAST_ANALYSIS_CALL r2m_binary_opq(THREADID tid, ADDRINT dst,
  ...
  tag_t res_tag[] = {
      tag_combine(dst_tag[0], src_tag[0]), tag_combine(dst_tag[1], src_tag[1]),
  ...
  tagmap_setb(dst, res_tag[0]);
  tagmap_setb(dst + 1, res_tag[1]);
  tagmap_setb(dst + 2, res_tag[2]);
  tagmap_setb(dst + 3, res_tag[3]);
  tagmap_setb(dst + 4, res_tag[4]);
  tagmap_setb(dst + 5, res_tag[5]);
  tagmap_setb(dst + 6, res_tag[6]);
  tagmap_setb(dst + 7, res_tag[7]);
}
...
static void PIN_FAST_ANALYSIS_CALL r_clrl4(THREADID tid) {
  for (size_t i = 0; i < 8; i++) {
    RTAG(tid)[DFT_REG_RDX][i] = tag_traits<tag_t>::cleared_val;
    RTAG(tid)[DFT_REG_RCX][i] = tag_traits<tag_t>::cleared_val;
    RTAG(tid)[DFT_REG_RBX][i] = tag_traits<tag_t>::cleared_val;
    RTAG(tid)[DFT_REG_RAX][i] = tag_traits<tag_t>::cleared_val;
  }
}
...
static void PIN_FAST_ANALYSIS_CALL r_clrx(THREADID tid, uint32_t reg) {
  for (size_t i = 0; i < 16; i++) {
    RTAG(tid)[reg][i] = tag_traits<tag_t>::cleared_val;
  }
}
...
static void PIN_FAST_ANALYSIS_CALL r_clrb_u(THREADID tid, uint32_t reg) {
  RTAG(tid)[reg][1] = tag_traits<tag_t>::cleared_val;
}

static void PIN_FAST_ANALYSIS_CALL r_clrb_l(THREADID tid, uint32_t reg) {
  RTAG(tid)[reg][0] = tag_traits<tag_t>::cleared_val;
}
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opb_lu(THREADID tid, uint32_t dst,
                                                   uint32_t src) {
  tag_t src_tag = RTAG(tid)[src][1];
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opb_u(THREADID tid, uint32_t dst,
                                                  uint32_t src) {
  tag_t src_tag = RTAG(tid)[src][1];
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opb_l(THREADID tid, uint32_t dst,
                                                  uint32_t src) {
  tag_t src_tag = RTAG(tid)[src][0];
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opw(THREADID tid, uint32_t dst,
                                                uint32_t src) {
  tag_t src_tag[] = R16TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_idx_xfer_opw(ADDRINT ins_address,
                                                        THREADID tid,
                                                        uint32_t dst,
                                                        uint32_t src) {
  tag_t src_tag[] = R16TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_base_xfer_opw(ADDRINT ins_address,
                                                         THREADID tid,
                                                         uint32_t dst,
                                                         uint32_t src) {
  tag_t src_tag[] = R16TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opq(THREADID tid, uint32_t dst,
                                                uint32_t src) {
  tag_t src_tag[] = R64TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_xfer_opx(THREADID tid, uint32_t dst,
                                                uint32_t src) {
  tag_t src_tag[] = R128TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_idx_xfer_opq(ADDRINT ins_address,
                                                        THREADID tid,
                                                        uint32_t dst,
                                                        uint32_t src) {
  tag_t src_tag[] = R64TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_idx_xfer_opl(ADDRINT ins_address,
                                                        THREADID tid,
                                                        uint32_t dst,
                                                        uint32_t src) {
  tag_t src_tag[] = R32TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_base_xfer_opq(ADDRINT ins_address,
                                                         THREADID tid,
                                                         uint32_t dst,
                                                         uint32_t src) {
  tag_t src_tag[] = R64TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2r_lea_base_xfer_opl(ADDRINT ins_address,
                                                         THREADID tid,
                                                         uint32_t dst,
                                                         uint32_t src) {
  tag_t src_tag[] = R32TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opb_u(THREADID tid, uint32_t dst,
                                                  ADDRINT src) {
  tag_t src_tag = MTAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opb_l(THREADID tid, uint32_t dst,
                                                  ADDRINT src) {
  tag_t src_tag = MTAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opw(THREADID tid, uint32_t dst,
                                                ADDRINT src) {
  tag_t src_tag[] = M16TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opq_h(THREADID tid, uint32_t dst,
                                                  ADDRINT src) {
  tag_t src_tag[] = M64TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opq(THREADID tid, uint32_t dst,
                                                ADDRINT src) {
  tag_t src_tag[] = M64TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2r_xfer_opx(THREADID tid, uint32_t dst,
                                                ADDRINT src) {
  tag_t src_tag[] = M128TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opb_u(THREADID tid, ADDRINT dst,
                                                  uint32_t src) {
  tag_t src_tag = RTAG(tid)[src][1];
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opb_l(THREADID tid, ADDRINT dst,
                                                  uint32_t src) {
  tag_t src_tag = RTAG(tid)[src][0];
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opw(THREADID tid, ADDRINT dst,
                                                uint32_t src) {
  tag_t src_tag[] = R16TAG(tid, src);

  tagmap_setb(dst, src_tag[0]);
  tagmap_setb(dst + 1, src_tag[1]);
}
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opq_h(THREADID tid, ADDRINT dst,
                                                  uint32_t src) {
  tag_t src_tag[] = R128TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opq(THREADID tid, ADDRINT dst,
                                                uint32_t src) {
  tag_t src_tag[] = R64TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opx(THREADID tid, ADDRINT dst,
                                                uint32_t src) {
  tag_t src_tag[] = R128TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL r2m_xfer_opl(THREADID tid, ADDRINT dst,
                                                uint32_t src) {
  tag_t src_tag[] = R32TAG(tid, src);
...
static void PIN_FAST_ANALYSIS_CALL m2m_xfer_opw(ADDRINT dst, ADDRINT src) {
  tag_t src_tag[] = M16TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2m_xfer_opb(ADDRINT dst, ADDRINT src) {
  tag_t src_tag = MTAG(src);

  tagmap_setb(dst, src_tag);
}

static void PIN_FAST_ANALYSIS_CALL m2m_xfer_opq(ADDRINT dst, ADDRINT src) {
  tag_t src_tag[] = M64TAG(src);
...
static void PIN_FAST_ANALYSIS_CALL m2m_xfer_opl(ADDRINT dst, ADDRINT src) {
  tag_t src_tag[] = M32TAG(src);
...
void ins_inspect(INS ins) {
    ...
    if (INS_OperandIsImmediate(ins, OP_1) ||
      ...
      if (INS_OperandIsMemory(ins, OP_0)) {
        ...
        case MEM_BYTE_LEN:
          ...
          break;
        default:
          LOG(string(__func__) + ": unhandled operand width (" +
              INS_Disassemble(ins) + ")\n");
      ...
      if (REG_is_gr64(reg_dst))
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else if (REG_is_gr8(reg_dst)) {
        if (REG_is_Lower8(reg_dst) && REG_is_Lower8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_Upper8(reg_dst) && REG_is_Upper8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_Lower8(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opb_lu,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opb_ul,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      }
      ...
      reg_dst = INS_OperandReg(ins, OP_0);
      if (REG_is_gr64(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else if (REG_is_gr16(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else if (REG_is_Upper8(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opb_u,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opb_l,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
    } else {
      reg_src = INS_OperandReg(ins, OP_1);
      ...
      if (REG_is_gr64(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else if (REG_is_gr32(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else if (REG_is_gr16(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else if (REG_is_Upper8(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opb_u,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opb_l,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
    }
  ...
  case XED_ICLASS_CMOVZ:
    if (INS_MemoryOperandCount(ins) == 0) {
      ...
      if (REG_is_gr64(reg_dst))
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opq,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                                 REG_INDX(reg_src), IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opl,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                                 REG_INDX(reg_src), IARG_END);
      else
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opw,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                                 REG_INDX(reg_src), IARG_END);
    } else {
      reg_dst = INS_OperandReg(ins, OP_0);
      if (REG_is_gr64(reg_dst))
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opq,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst),
                                 IARG_MEMORYREAD_EA, IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opl,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst),
                                 IARG_MEMORYREAD_EA, IARG_END);
      else
        INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opw,
                                 IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                 IARG_UINT32, REG_INDX(reg_dst),
                                 IARG_MEMORYREAD_EA, IARG_END);
    }
  ...
  case XED_ICLASS_CBW:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opb_ul,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                   REG_INDX(REG_AH), IARG_UINT32, REG_INDX(REG_AL), IARG_END);

    break;
  case XED_ICLASS_CWD:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opw,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                   REG_INDX(REG_DX), IARG_UINT32, REG_INDX(REG_AX), IARG_END);

    break;
  case XED_ICLASS_CWDE:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_cwde, IARG_FAST_ANALYSIS_CALL,
                   IARG_THREAD_ID, IARG_END);
    break;
  case XED_ICLASS_CDQ:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opl,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                   REG_INDX(REG_EDX), IARG_UINT32, REG_INDX(REG_EAX), IARG_END);

    break;
  case XED_ICLASS_CDQE:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_cdqe, IARG_FAST_ANALYSIS_CALL,
                   IARG_THREAD_ID, IARG_END);
    break;
  case XED_ICLASS_CQO:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opq,
    ...
    if (INS_MemoryOperandCount(ins) == 0) {
      ...
      if (REG_is_gr16(reg_dst)) {
        if (REG_is_Upper8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opwb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opwb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      } else if (REG_is_gr16(reg_src)) {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opqw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_oplw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        ...
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opqb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_oplb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      } else {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opqb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_oplb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      }
      ...
      reg_dst = INS_OperandReg(ins, OP_0);
      if (REG_is_gr16(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_opwb,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else if (INS_MemoryWriteSize(ins) == BIT2BYTE(MEM_WORD_LEN)) {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_opqw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_oplw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);

      } else {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_opqb,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_oplb,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      }
    }
  ...
  case XED_ICLASS_MOVSXD:
    if (INS_MemoryOperandCount(ins) == 0) {
      reg_dst = INS_OperandReg(ins, OP_0);
      reg_src = INS_OperandReg(ins, OP_1);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_r2r_opql,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                     IARG_END);
    } else {
      reg_dst = INS_OperandReg(ins, OP_0);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movsx_m2r_opql,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
    }
    break;
  case XED_ICLASS_MOVZX:
    if (INS_MemoryOperandCount(ins) == 0) {
      ...
      if (REG_is_gr16(reg_dst)) {
        if (REG_is_Upper8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_opwb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_opwb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      } else if (REG_is_gr16(reg_src)) {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_opqw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_oplw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      } else if (REG_is_Upper8(reg_src)) {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_opqb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_oplb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      } else {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_opqb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_r2r_oplb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      }
      ...
      if (REG_is_gr16(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_m2r_opwb,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      else if (INS_MemoryWriteSize(ins) == BIT2BYTE(MEM_WORD_LEN)) {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_m2r_opqw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_m2r_oplw,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      } else {
        if (REG_is_gr64(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_m2r_opqb,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
        else if (REG_is_gr32(reg_dst))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_movzx_m2r_oplb,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
      }
    }
  ...
  case XED_ICLASS_MUL:
    if (INS_OperandIsMemory(ins, OP_0))
      ...
      case BIT2BYTE(MEM_WORD_LEN):
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_ternary_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYREAD_EA, IARG_END);
      ...
      default:
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_ternary_opb,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_MEMORYREAD_EA, IARG_END);
        break;
      }
      ...
      if (REG_is_gr64(reg_src))
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_ternary_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_END);
      else if (REG_is_Upper8(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_ternary_opb_u,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_ternary_opb_l,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_END);
    }
    break;
  ...
  case XED_ICLASS_RDTSC:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r_clrl2,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);

    break;
  case XED_ICLASS_CPUID:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r_clrl4,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);
    ...
    if (INS_MemoryOperandCount(ins) == 0) {
      ...
      if (REG_is_gr64(reg_dst)) {
        INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)_cmpxchg_r2r_opq_fast,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                         IARG_REG_VALUE, REG_EAX, IARG_UINT32,
        ...
        INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)_cmpxchg_r2r_opw_fast,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                         IARG_REG_VALUE, REG_AX, IARG_UINT32, REG_INDX(reg_dst),
        ...
        LOG(string(__func__) +
            ": unhandled opcode (opcode=" + decstr(ins_indx) + ")\n");
    } else {
      reg_src = INS_OperandReg(ins, OP_1);
      if (REG_is_gr64(reg_src)) {
        INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)_cmpxchg_m2r_opq_fast,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                         IARG_REG_VALUE, REG_EAX, IARG_MEMORYREAD_EA, IARG_END);
        ...
        INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)_cmpxchg_m2r_opw_fast,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                         IARG_REG_VALUE, REG_AX, IARG_MEMORYREAD_EA, IARG_END);
  ...
  case XED_ICLASS_XADD:
    if (INS_MemoryOperandCount(ins) == 0) {
      ...
      if (REG_is_gr64(reg_dst)) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_r2r_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                       IARG_END);
      else if (REG_is_gr8(reg_dst)) {
        if (REG_is_Lower8(reg_dst) && REG_is_Lower8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_r2r_opb_l,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_Upper8(reg_dst) && REG_is_Upper8(reg_src))
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_r2r_opb_u,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else if (REG_is_Lower8(reg_dst))

          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_r2r_opb_lu,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
        else
          INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_r2r_opb_ul,
                         IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                         REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
                         IARG_END);
      }
      ...
      reg_src = INS_OperandReg(ins, OP_1);
      if (REG_is_gr64(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_m2r_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_MEMORYWRITE_EA, IARG_END);
      if (REG_is_gr32(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_m2r_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_MEMORYWRITE_EA, IARG_END);
      else if (REG_is_gr16(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_m2r_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_MEMORYWRITE_EA, IARG_END);
      else if (REG_is_Upper8(reg_src))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_m2r_opb_u,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_MEMORYWRITE_EA, IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_xadd_m2r_opb_l,
                       IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                       REG_INDX(reg_src), IARG_MEMORYWRITE_EA, IARG_END);
    }
  ...
  case XED_ICLASS_XLAT:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opb_l,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                   REG_INDX(REG_AL), IARG_MEMORYREAD_EA, IARG_END);

    break;
  case XED_ICLASS_LODSB:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opb_l,
                             IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                             IARG_UINT32, REG_INDX(REG_AL), IARG_MEMORYREAD_EA,
                             IARG_END);
  ...
  case XED_ICLASS_LODSW:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opw,
                             IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                             IARG_UINT32, REG_INDX(REG_AX), IARG_MEMORYREAD_EA,
                             IARG_END);
  ...
  case XED_ICLASS_LODSD:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opl,
                             IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                             IARG_UINT32, REG_INDX(REG_EAX), IARG_MEMORYREAD_EA,
                             IARG_END);
  ...
  case XED_ICLASS_STOSB:
    if (INS_RepPrefix(ins)) {
      ...
      INS_InsertThenPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opbn,
                                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_REG_VALUE,
                                   INS_RepCountRegister(ins), IARG_REG_VALUE,
                                   INS_OperandReg(ins, OP_4), IARG_END);
    } else
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opb_l,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                     IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(REG_AL),
                     IARG_END);
  ...
  case XED_ICLASS_STOSW:
    if (INS_RepPrefix(ins)) {
      INS_InsertIfPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)rep_predicate,
                                 IARG_FAST_ANALYSIS_CALL,
                                 IARG_FIRST_REP_ITERATION, IARG_END);
      INS_InsertThenPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opwn,
                                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_REG_VALUE,
                                   INS_RepCountRegister(ins), IARG_REG_VALUE,
                                   INS_OperandReg(ins, OP_4), IARG_END);
      ...
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opw,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                     IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(REG_AX),
                     IARG_END);
  ...
  case XED_ICLASS_STOSD:
    if (INS_RepPrefix(ins)) {
      INS_InsertIfPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)rep_predicate,
                                 IARG_FAST_ANALYSIS_CALL,
                                 IARG_FIRST_REP_ITERATION, IARG_END);
      INS_InsertThenPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opln,
                                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_REG_VALUE,
                                   INS_RepCountRegister(ins), IARG_REG_VALUE,
                                   INS_OperandReg(ins, OP_4), IARG_END);
      ...
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opl,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                     IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(REG_EAX),
                     IARG_END);
  ...
  case XED_ICLASS_STOSQ:
    if (INS_RepPrefix(ins)) {
      INS_InsertIfPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)rep_predicate,
                                 IARG_FAST_ANALYSIS_CALL,
                                 IARG_FIRST_REP_ITERATION, IARG_END);
      INS_InsertThenPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opqn,
                                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_REG_VALUE,
                                   INS_RepCountRegister(ins), IARG_REG_VALUE,
                                   INS_OperandReg(ins, OP_4), IARG_END);
  ...
  case XED_ICLASS_MOVSQ:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opq,
                             IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                             IARG_MEMORYREAD_EA, IARG_END);

    break;
  case XED_ICLASS_MOVSD:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opl,
                             IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                             IARG_MEMORYREAD_EA, IARG_END);

    break;
  case XED_ICLASS_MOVSW:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opw,
                             IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                             IARG_MEMORYREAD_EA, IARG_END);

    break;
  case XED_ICLASS_MOVSB:
    INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opb,
                             IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                             IARG_MEMORYREAD_EA, IARG_END);
  ...
  case XED_ICLASS_POP:
    if (INS_OperandIsReg(ins, OP_0)) {
      ...
      if (INS_MemoryWriteSize(ins) == BIT2BYTE(MEM_64BIT_LEN))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                       IARG_MEMORYREAD_EA, IARG_END);

      else if (INS_MemoryWriteSize(ins) == BIT2BYTE(MEM_LONG_LEN))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                       IARG_MEMORYREAD_EA, IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2m_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_MEMORYWRITE_EA,
                       IARG_MEMORYREAD_EA, IARG_END);
    }
  ...
  case XED_ICLASS_POPA:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_restore_opw,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD_EA,
                   IARG_END);

    break;
  case XED_ICLASS_POPAD:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_restore_opl,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYREAD_EA,
                   IARG_END);

    break;
  case XED_ICLASS_PUSHA:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_save_opw,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYWRITE_EA,
                   IARG_END);

    break;
  case XED_ICLASS_PUSHAD:
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_save_opl,
                   IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_MEMORYWRITE_EA,
                   IARG_END);
  ...
  case XED_ICLASS_LEAVE:
    ...
    if (REG_is_gr64(reg_dst)) {
      ...
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opq,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_src), IARG_MEMORYREAD_EA, IARG_END);
    } else if (REG_is_gr32(reg_dst)) {
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opl,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_src),
  ...
  case XED_ICLASS_LEA:
    ...
    if (reg_base != REG_INVALID() && reg_indx == REG_INVALID()) {
      if (REG_is_gr64(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_base_xfer_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_base_xfer_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_base_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_END);
    }
    if (reg_base == REG_INVALID() && reg_indx != REG_INVALID()) {
      if (REG_is_gr64(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_idx_xfer_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_indx), IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_idx_xfer_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_indx), IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_lea_idx_xfer_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_indx), IARG_END);
    }
    if (reg_base != REG_INVALID() && reg_indx != REG_INVALID()) {
      if (REG_is_gr64(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_lea_r2r_opq,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_UINT32, REG_INDX(reg_indx),
                       IARG_END);
      else if (REG_is_gr32(reg_dst))
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_lea_r2r_opl,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_UINT32, REG_INDX(reg_indx),
                       IARG_END);
      else
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)_lea_r2r_opw,
                       IARG_FAST_ANALYSIS_CALL, IARG_INST_PTR, IARG_THREAD_ID,
                       IARG_UINT32, REG_INDX(reg_dst), IARG_UINT32,
                       REG_INDX(reg_base), IARG_UINT32, REG_INDX(reg_indx),
                       IARG_END);
  ...
  case XED_ICLASS_MOVUPD:
    if (INS_MemoryOperandCount(ins) == 0) {
      reg_dst = INS_OperandReg(ins, OP_0);
      reg_src = INS_OperandReg(ins, OP_1);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2r_xfer_opx,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_UINT32, REG_INDX(reg_dst),
                     IARG_END);
    } else if (INS_OperandIsReg(ins, OP_0)) {
      reg_dst = INS_OperandReg(ins, OP_0);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opx,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
    } else {
      reg_src = INS_OperandReg(ins, OP_1);
  ...
  case XED_ICLASS_MOVHPS:
    if (INS_OperandIsMemory(ins, OP_0)) {
      reg_src = INS_OperandReg(ins, OP_1);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)r2m_xfer_opq_h,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                     IARG_MEMORYWRITE_EA, IARG_UINT32, REG_INDX(reg_src),
                     IARG_END);
      ...
      reg_dst = INS_OperandReg(ins, OP_0);
      INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)m2r_xfer_opq_h,
                     IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_UINT32,
                     REG_INDX(reg_dst), IARG_MEMORYREAD_EA, IARG_END);
    }
    break;
