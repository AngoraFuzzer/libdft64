...
void ins_ternary_op(INS ins) {
  if (INS_OperandIsMemory(ins, OP_0))
    ...
    case BIT2BYTE(MEM_64BIT_LEN):
      M_CALL_R(m2r_ternary_opq);
      break;
    case BIT2BYTE(MEM_LONG_LEN):
      M_CALL_R(m2r_ternary_opl);
      break;
    case BIT2BYTE(MEM_WORD_LEN):
      M_CALL_R(m2r_ternary_opw);
      break;
    case BIT2BYTE(MEM_BYTE_LEN):
    default:
      M_CALL_R(m2r_ternary_opb);
      break;
    }
  else {
    REG reg_src = INS_OperandReg(ins, OP_0);
    if (REG_is_gr64(reg_src))
      R_CALL(r2r_ternary_opq, reg_src);
    else if (REG_is_gr32(reg_src))
      R_CALL(r2r_ternary_opl, reg_src);
    else if (REG_is_gr16(reg_src))
      R_CALL(r2r_ternary_opw, reg_src);
    else if (REG_is_Upper8(reg_src))
      R_CALL(r2r_ternary_opb_u, reg_src);
    else
      R_CALL(r2r_ternary_opb_l, reg_src);
    }
}